...
const ManageHospitalNurses = () => {
    ...
    const fetchNurses = async () => {
        try {
            const { data } = await api.get('/hospital/nurses');
            setNurses(data);
        } catch (err) {
            console.error(err);
        } finally {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm("Are you sure you want to remove this nurse from your hospital?")) return;
        try {
            await api.delete(`/hospital/nurses/${id}`);
    ...
    const handleEditSubmit = async (e) => {
        e.preventDefault();
        try {
            const { data } = await api.put(`/hospital/nurses/${editingNurse.id}`, editingNurse);
        ...
        <div className="manage-container">
            <div className="manage-header">
                <div className="manage-title">
                    <h2>Manage Nurses</h2>
                    <p>View, edit, or remove nurses within your hospital</p>
                </div>
...
export default ManageHospitalNurses;
