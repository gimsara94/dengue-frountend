...
const CreateHospitalDoctor = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setStatus({ type: 'success', message: 'Doctor registered successfully for your hospital!' });
            setFormData({ name: '', telephone: '', nic_no: '', address: '', email: '', username: '', password: '' });
        } catch (err) {
            setStatus({
                type: 'error',
                message: err.response?.data?.message || 'Error creating doctor.'
            });
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="admin-form-container" style={{ maxWidth: '800px', margin: '0 auto' }}>
            <div className="admin-form-header">
                <h2 className="admin-form-title">Register Doctor</h2>
                <p className="admin-form-subtitle">Add a new medical professional to your hospital staff.</p>
            </div>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <h3 style={{ fontSize: '1.1rem', color: '#0f172a', fontWeight: '600', marginBottom: '1rem' }}>Login Credentials</h3>
...
export default CreateHospitalDoctor;
